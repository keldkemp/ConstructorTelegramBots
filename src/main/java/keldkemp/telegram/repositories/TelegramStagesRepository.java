...
public interface TelegramStagesRepository extends JpaRepository<TelegramStages, Long> {
    ...
    @Query("select stage from TelegramStages stage where stage.telegramBot.id = :botId and stage.previousStage is null")
    TelegramStages getFirstStage(@Param("botId") Long botId);
